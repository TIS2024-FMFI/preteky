...
import HandlerOfInputsFromUi as hoi
...
def preteky_z_SQLite():
    ...
    '''
    return [
        {"datum": f"2023-{i ** i % 12 + 1}-0{i + 1}", "nazov": f"Race {i + 1}",
...
def zoznam_pretekov_isOr(called_from, mesiac=None):
    ...
    while True:
        ...
        preteky_display = [f"{p['datum']} | {p['nazov']} | {p['deadline']} | {p['miesto']} | {p['kategorie']}" for p in
                           preteky]
        ...
        if key == b'H' and current_idx > 0:  # Up
            ...
            if preteky_display[current_idx] == "Back":
                ...
                log.insert(0,
                           f"Response from isOrienteering -> SQLite was {import_preteku_isOr_API_do_databazy(preteky_display[current_idx])}")
